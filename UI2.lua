...
local TextLabel1 = Custom:Create("TextLabel", {
  ...
  local NameTab = Custom:Create("TextLabel", {
	...
	Position = UDim2.new(0, 10, 0, 0), -- geser 50px ke kanan
    Name = "NameTab"
  }, Layers)
...
function Item:AddDropdown(Config)
    ...
    function Funcs_Dropdown:AddOption(OptionName)
        ...
        Custom:Create("UICorner", { CornerRadius = UDim.new(0,14) }, ChooseFrame)

        OptionButton.Activated:Connect(function()
            -- Single select: hide others
            if not Multi then
                ...
                -- Multi select toggle
                local exists = table.find(Funcs_Dropdown.Value, OptionName)
                if exists then
                    table.remove(Funcs_Dropdown.Value, exists)
            ...
            OptionSelecting.Text = table.concat(Funcs_Dropdown.Value, ", ")
            if not Multi then
                Callback(Funcs_Dropdown.Value[1])
            else
    ...
    function Funcs_Dropdown:Set(Value)
        ...
        Funcs_Dropdown.Value = type(Value) == "table" and Value or {Value}
        OptionSelecting.Text = table.concat(Funcs_Dropdown.Value, ", ")

        -- Set choose frame visibility
        for _, child in pairs(ScrollSelect:GetChildren()) do
            if child.Name == "Option" then
                local cf = child:FindFirstChild("ChooseFrame")
                local txt = child:FindFirstChildWhichIsA("TextLabel")
                if cf and txt then
                    cf.Visible = table.find(Funcs_Dropdown.Value, txt.Text) and true or false
                end
            end
        end
    end
    ...
    function Funcs_Dropdown:Refresh(List, DefaultVal)
        self:Clear()
        for _, opt in ipairs(List or Options) do
            self:AddOption(opt)
        end
        self.Options = List or Options
        self:Set(DefaultVal or self.Value)
    end

    -- Initialize
    Funcs_Dropdown:Refresh(Options, Default)

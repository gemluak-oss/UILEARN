    ...
    function self:CreateModal(title, fields)
        ...
        for _, field in ipairs(fields) do
            ...
            elseif field.Type == "toggle" then
                local default = field.Default or false
                local entry = {Name = field.Name, Value = default}
                ...
                AddCorner(row, 4)
                AddStroke(row, currentTheme.Border, 1)
                ...
                local togBg = Create("Frame", {
                    Parent = row,
                    BackgroundColor3 = default and currentTheme.Blueprint or currentTheme.Paper,
                ...
                row.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        state = not state
                        entry.Value = state
                        Tween(togBg, {BackgroundColor3 = state and currentTheme.Blueprint or currentTheme.Paper}, 0.15)
            ...
            elseif field.Type == "slider" then
                ...
                local default = field.Default or min
                local entry = {Name = field.Name, Value = default}
                table.insert(results, entry)
                ...
                barBg.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        conn = RunService.RenderStepped:Connect(function()
                            local mouse = UserInputService:GetMouseLocation()
                            local perc = math.clamp((mouse.X - barBg.AbsolutePosition.X) / barBg.AbsoluteSize.X, 0, 1)
                            local val = math.round(min + (max - min) * perc)
                            entry.Value = val
                            barFill.Size = UDim2.new(perc, 0, 1, 0)
                            valueLabel.Text = tostring(val)
                        end)
            ...
            elseif field.Type == "dropdown" then
                ...
                local default = field.Default or options[1]
                local entry = {Name = field.Name, Value = default}
                table.insert(results, entry)
                ...
                AddStroke(cycleBtn, currentTheme.Border, 1)

                local idx = table.find(options, default) or 1
                cycleBtn.MouseButton1Click:Connect(function()
                    idx = idx % #options + 1
                    entry.Value = options[idx]
                    cycleBtn.Text = tostring(options[idx]) .. "  ▼"
                end)
